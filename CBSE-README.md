# Hyperparameter Optimization using CPU and GPU

In this code pattern we demonstrate how to build a multi class classification neural network model and optimize the hyperparameters to enhance the accuracy. The content includes creation and deployment of deep learning models using Jupyter Notebook (CPU based) in Watson Studio environment and create deep learning `Experiments` (using GPU) with hyper parameters optimization using Watson Studio GUI for monitoring different runs and select the best model for deployment. 
...
1. [Evaluate model performance](#7-evaluate-model-performance)
